...
//	userNotification.contentImage = [NSApp applicationIconImage];
//	userNotification.soundName = @"digit";
	...
	if (![webView.window isMainWindow] || _count) {
		_count++;
		[[[NSApplication sharedApplication] dockTile] setBadgeLabel:[[NSString alloc] initWithFormat:@"%ld", _count]];
	}
}
...
- (void)cancelNotification:(WebNotification *)webNotification {
	...
	if (userNotification)
		[_notificationCenter removeDeliveredNotification:userNotification];
